...
# ---------------- BUILD UI ----------------
def build_players_text(current_user_id, is_admin=False):
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    text, markup = build_players_text(update.effective_user.id, is_admin)
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if update.effective_chat.type == "private":
        msg = await context.bot.send_message(chat_id=chat_id, text="ℹ️ Press /start")
        schedule_delete(context, chat_id, msg.message_id, 5)
