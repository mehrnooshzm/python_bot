...
# ---------------- BUILD LIST ----------------
def build_players_text(current_user_id):
    ...
    for index, user in enumerate(users):
        ...
        # فقط اگر مال خودش بود edit کنه
        if user_id == current_user_id:
            buttons.append(
                InlineKeyboardButton(f"✏️ {username}", callback_data=f"edit_{row_id}")
            )

        buttons.append(InlineKeyboardButton("❌", callback_data=f"delete_{row_id}"))
...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # ---------------- LIST ----------------
        ...
        text, markup = build_players_text(user_id)
    ...
    # ---------------- DELETE ----------------
        ...
        text, markup = build_players_textuser()
