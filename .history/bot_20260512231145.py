...
ADMIN_IDS = [123456789]  # 👈 اینو با آیدی خودت عوض کن

waiting_users = set()
waiting_jobs = {}
waiting_edit = {}  # user_id -> row_id OR "setup"
...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # ---------------- SETUP (ADMIN ONLY) ----------------
        ...
        waiting_edit[user_id] = "setup"

        await context.bot.send_message(
            chat_id=chat_id, text="📌 Send tournament info:\nname | date | location"
        )
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # ---------------- SETUP MODE ----------------
    if user_id in waiting_edit and waiting_edit[user_id] == "setup":

        try:
            name, date, location = text.split("|")

            database.set_value("name", name.strip())
            database.set_value("date", date.strip())
            database.set_value("location", location.strip())
            ...
            await context.bot.send_message(
                chat_id=chat_id, text="❌ Format: name | date | location"
            )
