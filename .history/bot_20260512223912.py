...
waiting_prompt_msg = {}  # user_id -> message_id  ✅ NEW
...
async def timeout_user(context: ContextTypes.DEFAULT_TYPE):
    ...
    if user_id in waiting_users:
        ...
        timeout_msg = await context.bot.send_message(
            chat_id=chat_id, text="⏰ Time is finished. again press Register"
        )
...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if query.data == "register":
        ...
        # store prompt message id  ✅ NEW
        waiting_prompt_msg[user_id] = msg.message_id
        ...
        job = context.job_queue.run_once(
            ...
            data={
                "user_id": user_id,
                "chat_id": query.message.chat.id,  # FIXED ❌ chat.iad -> id
    ...
    # ---------------- LIST ----------------
        ...
        await query.message.edit_text(
            ...
            reply_markup=InlineKeyboardMarkup(
                [
                    ...
                    [
                        InlineKeyboardButton(
                            "❌ Delete Name", callback_data="delete_name"
                        )
                    ],
                ]
            ),
        ...
        confirm = await query.message.reply_text("❌ Your name was deleted.")
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if user_id in waiting_users:
        ...
        # ❌ delete prompt message ("Send your name...")
        if user_id in waiting_prompt_msg:
            try:
                await context.bot.delete_message(
                    chat_id=update.effective_chat.id,
                    message_id=waiting_prompt_msg[user_id],
                )
            except Exception as e:
                print("prompt delete error:", e)

            del waiting_prompt_msg[user_id]
